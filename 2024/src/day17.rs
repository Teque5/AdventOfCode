...
/// run *dv instruction
fn dv(reg_a: usize, reg_b: usize, reg_c: usize, operand: usize) -> usize {
    let numer = reg_a as f64;
    let denom = (2.0_f64).powf(co(operand, reg_a, reg_b, reg_c) as f64);
    (numer / denom) as usize
}
...
fn part(filename: &str, is_part1: bool) -> String {
    ...
    loop {
        ...
        while point < program.len() - 1 {
            ...
            match instruction {
                0 => reg_a = dv(reg_a, reg_b, reg_c, operand), // adv
                1 => reg_b ^= operand,                         // bxl
                2 => reg_b = co(operand, reg_a, reg_b, reg_c) % 8, // bst
                3 => {}                                        // jnz handled down further
                4 => reg_b = reg_b ^ reg_c,                    // bxc
                5 => {
                    output.push(co(operand, reg_a, reg_b, reg_c) % 8);
                    is_output = true;
                } // out
                6 => reg_b = dv(reg_a, reg_b, reg_c, operand), // bdv
                7 => reg_c = dv(reg_a, reg_b, reg_c, operand), // cdv
                _ => unreachable!(),
            }
            // pointer math
            if instruction == 3 && reg_a != 0 {
                // jnz increment
                point = operand;
            } else {
                // normal increment
...
// if you compute the diffs of these values you find that a spacing of 1073741824 is common.
// then do `sorted(list(set([int(item) % 1073741824 for item in brute_force])))` and you have 22 offsets to try
